...
type Arc struct {
	...
	INHIBITOR int    `json:"inhibitor"` // ! Added INHIBITOR attribute
	// ! For colored nets the inhibitor arc has the type:
	// ! map[string]int
}
...
func compareArc(marking Marking, arc Arc) bool {
	return (marking.CONTENT[arc.PLACE] < arc.VALUE) || (arc.INHIBITOR < marking.CONTENT[arc.PLACE]) // ! Added condition
}

// ! compareArc function for colored nets
// func compareArc(marking Marking, arc Arc) bool {
// 	ok := false
	...
	for transition := range net.TRANSITIONS {
		...
		for _, arc := range net.TRANSITIONS[transition].INPUT {
			if compareArc(marking, arc) {
				//fmt.Println(marking.CONTENT[arc.PLACE], arc.VALUE, arc.INHIBITOR)
				ok = false
				break
			}
		}
